...
std::string SHA1::final(bool binary)
{
    ...
    if (binary)
    {
        ...
        const std::string ret((char*) &digest[0], sizeof(digest));
        /* Reset for next run */
        reset(digest, buffer, transforms);
        return ret;
    }
...
std::string SHA1::from_file(const std::string &filename, bool binary)
{
    std::ifstream stream(filename.c_str(), std::ios::binary);
    SHA1 checksum;
    checksum.update(stream);
    return checksum.final(binary);
}
